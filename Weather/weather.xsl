...
<xsl:template name="temperature">
	<p class="temp"><xsl:value-of select="metric"/>°<span class="feel"><xsl:value-of select="metric"/>°</span></p>	
</xsl:template>
...
<xsl:template match="pop">
	<p> Probabilité de précipitation <xsl:value-of select="."/></p>
</xsl:template>
...
<xsl:template match="humidity">
	<p>Humidité : <xsl:value-of select="."/>%</p>
</xsl:template>
